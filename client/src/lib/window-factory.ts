// WindoorDesigner - 窗口工厂函数
// 负责创建各种预设窗型和基础窗口对象

import { nanoid } from 'nanoid';
import type { WindowUnit, Opening, Frame, Sash, ProfileSeries, Rect, Mullion, WindowTemplate, SashType } from './types';
...
export function createSash(type: SashType, rect: Rect): Sash {
  return {
    id: nanoid(8),
    type,
    rect,
...
export function createMullion(type: 'vertical' | 'horizontal', position: number): Mullion {
  return {
    id: nanoid(8),
    type,
    position,
...
// Split an opening with a mullion
export function splitOpening(
  ...
  const mullion = createMullion(type, position);
...
// Find opening at a point (recursive)
...
// Update opening rects after mullion drag
...
function createFixedWindow(id: string, x: number, y: number, series: ProfileSeries): WindowUnit {
  ...
  opening.sash = createSash('fixed', opening.rect);
  return win;
}
...
function createSingleCasement(id: string, x: number, y: number, series: ProfileSeries): WindowUnit {
  ...
  const opening = win.frame.openings[0];
  opening.sash = createSash('casement-left', opening.rect);
  return win;
}
...
function createDoubleCasement(id: string, x: number, y: number, series: ProfileSeries): WindowUnit {
  ...
  splitResult.childOpenings[0].sash = createSash('casement-left', splitResult.childOpenings[0].rect);
  splitResult.childOpenings[1].sash = createSash('casement-right', splitResult.childOpenings[1].rect);
...
function createThreePanelWindow(id: string, x: number, y: number, series: ProfileSeries): WindowUnit {
  ...
  const pos2 = pw + (innerW * 2) / 3;

  const topOpening = win.frame.openings[0];
  // First split
  const split1 = splitOpening(topOpening, 'vertical', pos1, mw);
  // Second split on the right child
  ...
  split1.childOpenings[0].sash = createSash('casement-left', split1.childOpenings[0].rect);
  split2.childOpenings[0].sash = createSash('fixed', split2.childOpenings[0].rect);
  split2.childOpenings[1].sash = createSash('casement-right', split2.childOpenings[1].rect);
...
function createSlidingWindow(id: string, x: number, y: number, series: ProfileSeries): WindowUnit {
  ...
  const topOpening = win.frame.openings[0];
  const splitResult = splitOpening(topOpening, 'vertical', midX, mw);
  splitResult.childOpenings[0].sash = createSash('sliding-left', splitResult.childOpenings[0].rect);
  splitResult.childOpenings[1].sash = createSash('sliding-right', splitResult.childOpenings[1].rect);
  win.frame.openings = [splitResult];
  return win;
}
...
function createTopHungWindow(id: string, x: number, y: number, series: ProfileSeries): WindowUnit {
  ...
  const opening = win.frame.openings[0];
  opening.sash = createSash('casement-top', opening.rect);
  return win;
}
