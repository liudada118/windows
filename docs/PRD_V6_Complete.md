...
| 5.5 Complete | 2026-03-02 | Manus AI | **根据 V3/V4 分析报告全面优化**：修复 EBNF 语法、统一主键规范、新增端到端流程图、WebSocket 事件目录、下料算法伪代码、打印模板引擎规格、数据模型映射规则、API 完整示例。**治愈结构腐化**（归位错位内容、合并重复章节、统一 API 路径）。**补齐 MVP 关键缺失**（边界校验、尺寸交互、颜色系统、型材切换）。**增强薄弱模块**（仓库/质量/财务管理）。**新增算料引擎调试工具**。 |
| 6.1 | 2026-03-04 | Manus AI | **精确输入功能**：实现属性面板点击编辑（EditableValue 组件）、画布双击输入（DimensionInput 组件）、键盘方向键微调三种精确输入入口。更新 7.17.2 和新增 7.11.3 章节。 |
| 6.0 Complete | 2026-03-02 | Manus AI | **结构重组**：通过脚本化操作将所有错位内容（3.44、7.17、8.5、9.3、11.1.2、12.4、14.1.4、19.24-28）归位到正确章节位置。删除 V5.4 结束标记和重复的 9.3 节。统一 API 路径为 `/api/v1/procurement/` 和 `/api/v1/qc/` 命名空间。合并修订历史重复记录。建立结构完整性 CI 校验脚本。 |
...
#### 7.11.3 精确输入交互状态机 (V6.1 新增)

```mermaid
graph TD
    A[显示态: span数字] -- 用户点击 --> B(编辑态: input输入框);
    B -- 输入合法值 + Enter --> C{校验};
    B -- Escape --> A;
    B -- blur --> C;
    C -- 通过 --> D[更新窗户数据];
...
| 状态 | 描述 |
| :--- | :--- |
| **显示态** | 数字以 `span` 渲染，带铅笔图标，鼠标悬停时 `cursor: pointer`，提示"点击编辑" |
| **编辑态** | 原地替换为 `input`，自动全选数值，边框高亮（`ring-2 ring-blue-500`） |
| **校验** | 检查输入值是否为正整数、是否在 `[MIN, MAX]` 范围内、分格修改是否导致相邻分格过窄 |
| **更新** | 调用 `resizeWindowUnit`（总宽高）或 `updateMullionInOpenings`（分格），触发画布重绘 |
...
本节描述已实现的精确数值输入功能，解决了"只能拖动不能输入"的核心痛点。

**入口一：属性面板精确输入（已实现）**

选中窗户后，右侧属性面板显示以下可编辑尺寸：

| 编辑项 | 显示位置 | 交互方式 | 联动行为 |
| :--- | :--- | :--- | :--- |
| **总宽度** | 属性面板 · 宽 | 点击数字 → 变为输入框 → 输入新值 → 回车确认 | 所有分格等比缩放，面积自动更新 |
| **总高度** | 属性面板 · 高 | 同上 | 所有分格高度同步更新，面积自动更新 |
| **分格宽度** | 属性面板 · 分格区域 | 点击分格宽度数字 → 输入新值 → 回车确认 | 中梃自动移动，相邻分格宽度自动补偿 |

**EditableValue 组件规格：**

- **空闲态**：显示为带铅笔图标的可点击数字（`span`），鼠标悬停时高亮提示"点击编辑"
- **编辑态**：点击后原地变为 `input[type=text]`，自动全选当前数值，聚焦等待输入
- **确认方式**：按 `Enter` 键确认，按 `Escape` 键取消，点击外部区域（blur）确认
- **校验规则**：输入值必须为正整数，范围受 `CONSTRAINTS.MIN_WINDOW_WIDTH/HEIGHT`（200mm）和 `CONSTRAINTS.MAX_WINDOW_WIDTH/HEIGHT`（6000/4000mm）约束
- **错误处理**：非法输入自动恢复为修改前的值，不产生任何副作用

**分格尺寸编辑的中梃联动算法：**

```
输入：targetOpeningIndex, newWidth
1. 计算 delta = newWidth - currentWidth
2. 找到该分格右侧的中梃（mullion）
3. 计算中梃新位置 = mullion.position + delta
4. 校验：新位置不得导致任何相邻分格宽度 < MIN_OPENING_WIDTH（100mm）
5. 校验通过 → 更新 mullion.position，重新计算所有 opening 的 x/width
6. 校验失败 → 恢复原值，不做任何修改
```

**入口二：画布双击输入（已实现）**

- 双击画布上的黄色尺寸标注数字 → 原地弹出 `DimensionInput` 浮层输入框
- 命中检测（`hitTestDimension`）：遍历所有窗户的宽度/高度/分格标注区域，检测双击坐标是否落在标注文字的 hitbox 内（±20px 容差）
- 输入确认后调用 `resizeWindowUnit` 或 `updateMullionInOpenings` 更新画布
...
#### PUT /api/v1/procurement/purchase-orders/{id}/status
